...
declare function GM_getValue(name: string, defaultValue: string): Promise<string>|string
declare function GM_getValue(name: string, defaultValue: number): Promise<number>|number
declare function GM_getValue(name: string, defaultValue: boolean): Promise<boolean>|boolean
declare function GM_setValue<T extends string|number|boolean>(name: string, value: T): Promise<T>|T
 ...
 */
async function shouldUpdate(listname: string, username: string, minMinuteDiff: number): Promise<boolean> {
    if (!SAVE_ENABLED) return true
    const now = Date.now()
    ...
    const minuteDiff = (now - then) / (60_000 /* number of milliseconds in 1 minute */)
...
function getUsername(): string|null {
    ...
    if (username) console.log(`Setting user: '${username}'`)
    return username
}
 ...
 */
async function retrieveAllUserHrefs(url: string): Promise<string[]> {
 ...
 */
async function retrieveUserBlockList(username: string): Promise<string[]> {
 ...
 * @param username The username to retrieve the blocklist of
 */
async function getUserBlockList(username: string): Promise<string[]> {
...
function tagWatchers(following_users: string[]) {
    console.log(`There are ${following_users.length} users watching ${ACTIVE_USER}`)
    if (following_users.length == 0) return

    function isWatcher(username: string|null): boolean {
...
function tagBlockedUsers(blocked_users: string[]) {
    console.log(`${ACTIVE_USER} has blocked ${blocked_users.length} users.`)
    if (blocked_users.length === 0) return

    function isBlocked(username: string|null): boolean {
